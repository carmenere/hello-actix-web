...
HOST ?= localhost
PORT ?= 5432
USER_DB ?= hello_actix_db
USER_NAME ?= hello_actix_user
USER_PASSWORD ?= 12345
...
CMD_DOC ?= $(BUILD_ENVS) cargo doc --no-deps --document-private-items --package hello-actix --open
...
build:
	cd $(PACKAGE) && $(CMD_BUILD)

clippy:
	cd $(PACKAGE) && $(CMD_CLIPPY)

clippy-fix:
	cd $(PACKAGE) && $(CMD_CLIPPY_FIX)
...
fmt-check:
	cd $(PACKAGE) && $(CMD_FMT_CHECK)

doc:
	cd $(PACKAGE) && $(CMD_DOC)

clean:
	cd $(PACKAGE) && $(CMD_CLEAN)
